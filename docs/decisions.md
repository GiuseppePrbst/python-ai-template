...
## ADR-012: capa de exploración y compactación estructurada para continuidad operativa (v0.3.1)

- **Fecha**: 2026-07-23
- **Estado**: aceptada (corregida tras la primera revisión).
- **Contexto**: tras ADR-001 el repositorio y `docs/` son la fuente de verdad operativa, y `docs/current-state.md` se reescribe con `/handoff` al cierre de cada bloque de trabajo. La operativa todavía dependía de (a) un único mecanismo de exploración sin contrato formal de solo lectura, (b) una revisión manual implícita sin atajo documentado y (c) la compactación automática de OpenCode, que podía perder el objetivo y las decisiones de la sesión al acercarse al límite. Tampoco existía un formato canónico único y verificable para `docs/current-state.md` que el resto de la operativa pudiera asumir. Una primera versión del plugin de compactación en esta misma ADR leía `docs/current-state.md` y `pyproject.toml` con `node:fs/promises`; la primera revisión interna y el criterio explícito del usuario vetaron esa lectura por contradecir la regla "no usa filesystem" y por añadir dependencia dinámica innecesaria. Esta ADR recoge ya el plugin estrictamente estático, sin filesystem, sin shell, sin red, sin logs y sin persistencia.
- **Decisión**: se introduce una **capa de exploración y compactación** que no toca el comportamiento del generador ni añade dependencias, formada por cuatro piezas:
  - **Agente `scout`** (`.opencode/agents/scout.md`). Exploración de solo lectura: no edita, no crea archivos, no instala dependencias, no ejecuta acciones destructivas, no decide arquitectura. Distingue explícitamente hechos, inferencias e hipótesis, cita rutas relativas y símbolos concretos, y devuelve un resumen compacto con las secciones: objetivo interpretado, hechos verificados, flujo actual, decisiones aplicables, riesgos, hipótesis, preguntas abiertas, archivos que probablemente requerirían cambios, siguiente lectura recomendada.
  - **Comando `/review`** (`.opencode/commands/review.md`). Atajo **manual** que lanza el agente `reviewer` sobre el diff vigente (staged, unstaged y untracked), lee archivos completos cuando el diff no basta, ejecuta los gates relevantes y emite un informe con secciones obligatorias (resumen, bloqueantes, mayores, menores, observaciones, validaciones ejecutadas, cambios fuera de alcance, veredicto). Los veredictos permitidos son `Aprobado`, `Aprobado con cambios menores`, `Requiere cambios` o `Rechazado`. **No** se automatiza mediante hooks porque OpenCode 1.18.4 expone `command.execute.before` pero no `command.execute.after`. Tras la primera revisión se añadió al comando un chequeo explícito: cuando el diff incluya nuevos plugins de compactación, debe verificar que no escriben archivos, no leen archivos vía filesystem, no usan shell, no llaman a la red, no generan logs permanentes, no modifican `output.prompt` y sólo agregan contexto auxiliar a `output.context`.
  - **Skill `context-handoff`** (`.opencode/skills/context-handoff/SKILL.md`). Define el formato canónico de `docs/current-state.md` con **diez secciones obligatorias**, en este orden: objetivo actual, estado de la tarea, hechos verificados, decisiones adoptadas, archivos modificados, validaciones ejecutadas, errores pendientes, enfoques rechazados y motivo, divergencias detectadas, siguiente acción concreta. El comando `/handoff` se alinea con este formato y deja de usar el formato previo de siete secciones.
  - **Plugin `structured-compaction`** (`.opencode/plugins/structured-compaction.ts`). Plugin TypeScript autodescubierto por OpenCode desde `.opencode/plugins/`. Registra **únicamente** el hook `experimental.session.compacting` y, en cada invocación, agrega **una sola entrada estática** (cadena literal definida como constante dentro del propio plugin) a `output.context`. La entrada enumera las diez secciones canónicas del handoff, distingue hechos de decisiones, hipótesis y pendientes, y recuerda los límites del resumen (sin secretos, sin logs completos, sin resultados inventados y sin tomar el resumen como autoridad documental). **Nunca** asigna `output.prompt`. **No** lee archivos: no usa filesystem, no importa módulos del sistema de archivos, no llama a `readFile`, `writeFile`, `readdir`, `stat`, `resolve` ni `join`. **No** ejecuta comandos: no usa la interfaz de shell que ofrece OpenCode. **No** usa la red. **No** genera logs permanentes ni escribe en flujos estándar. **No** persiste información. **No** modifica `docs/decisions.md` ni ningún otro archivo del repositorio. **No** intenta reconstruir el estado real del repositorio: deja esa reconstrucción al propio mecanismo nativo de compactación de OpenCode y al comando `/handoff`. El hook `experimental.session.compacting` es **experimental** en OpenCode 1.18.4; su forma o firma puede cambiar sin aviso. Si en el futuro deja de ser necesaria esta intervención, el plugin puede **eliminarse** sin reintroducir lógica dinámica, registrando la eliminación con una nueva ADR.
- **Alternativas consideradas**:
  - **No añadir el scout y reutilizar `reviewer` para exploración**: descartada porque el revisor modifica archivos cuando se le pide una corrección y no tiene contrato de solo lectura. Mezclar roles difumina la jerarquía vigente y abre la puerta a ediciones accidentales.
  - **Automatizar `/review` mediante un hook tras `command.execute.before`**: descartada porque OpenCode 1.18.4 no expone `command.execute.after`. Implementarlo mediante polling, eventos o shell sería fragil y contradictorio con el principio de no añadir dependencias. El diseño queda explícitamente como manual.
  - **Persistir el resumen de compactación en disco desde el plugin**: descartada porque el plugin no debe escribir archivos. La persistencia corresponde a `/handoff` y `docs/current-state.md`, fuera del plugin.
  - **Reemplazar `output.prompt` en lugar de añadir `output.context`**: descartada por el principio de "auxiliar, no autoridad". Reemplazar el prompt por defecto hace que el plugin asuma responsabilidad sobre el resumen; anexar a `output.context` deja la responsabilidad del resumen en OpenCode y limita al plugin a condicionar qué preservar.
  - **Plugin con shell (`BunShell`) para invocar `uv` o `git`**: descartada por las reglas operativas del proyecto: el plugin no ejecuta comandos.
  - **Plugin con lectura dinámica de `docs/current-state.md` y `pyproject.toml`**: fue la primera versión del plugin; se descartó tras la primera revisión por contradecir la regla explícita "no usa filesystem" y por añadir dependencia dinámica innecesaria. El plugin final es estrictamente estático y preserva su comportamiento sin observar el repositorio.
  - **Reintroducir un parser Markdown en el plugin para normalizar acentos**: rechazada por el principio de cambio mínimo; el plugin se mantiene como entrada literal. La robustez ante variantes ortográficas se delega al LLM, no al plugin.
  - **Introducir `tsconfig.json`, bundler o dependencias TypeScript**: descartada por el principio de cambio mínimo en la cadena de herramientas. El plugin se entrega como `.ts` que OpenCode ejecuta directamente con Bun; los tipos se importan de `@opencode-ai/plugin` 1.18.4, ya declarado en `.opencode/package.json`.
  - **Cavemem, OpenRouter, Caveman, Ponytail Ultra o routing adaptativo**: descartados por estar fuera del alcance de la unidad y por introducir proveedores o almacenes de memoria externos. La compactación estructurada se queda en el equipo del proyecto.
- **Consecuencias**:
  - `docs/current-state.md` pasa a tener diez secciones obligatorias. El bloque de trabajo v0.3.1 migra el archivo a ese formato; las sesiones que mantengan el formato previo quedan como históricas y deben reescribirse en el siguiente `/handoff`.
  - La exploración del repositorio se apoya en el agente `scout` con un contrato verificable. Las revisiones se canalizan por `/review` con un veredicto formal entre los cuatro permitidos. El agente `reviewer` sigue siendo el actor que juzga; el scout el que describe.
  - La compactación automática de OpenCode deja de ser opaca para las sesiones largas. El plugin `structured-compaction` se limita a condicionar al mecanismo nativo: enumera las diez secciones canónicas, distingue hechos, decisiones, hipótesis y pendientes, registra divergencias sin resolverlas y exige evitar secretos, logs completos y resultados inventados. El resumen final sigue siendo del mecanismo nativo de OpenCode; el plugin **no es autoridad documental** y **no actualiza `docs/current-state.md`** (esa responsabilidad corresponde a `/handoff`).
  - El plugin **no** lee `docs/current-state.md`, **no** lee `pyproject.toml`, **no** usa filesystem, **no** usa shell, **no** usa red, **no** genera logs permanentes, **no** persiste información y **no** reconstruye el estado del repositorio. Verificación mecánica: `grep -nE "node:fs|node:path|readFile|writeFile|readdir|stat\(" .opencode/plugins/structured-compaction.ts` devuelve cero coincidencias; `grep -c "output.context.push"` devuelve `1`; el plugin importa únicamente el tipo `Plugin` desde `@opencode-ai/plugin`.
  - Como el hook es **experimental**, cualquier cambio de firma en una versión futura de OpenCode obliga a una nueva ADR. El plugin no se modifica en silencio. Si el hook se renombra o se elimina, la opción por defecto es **eliminar el plugin** y dejar la compactación al mecanismo por defecto de OpenCode, registrando el cambio en una nueva ADR.
  ...
  - La política de compactación y contexto se actualiza en `docs/ai/context-policy.md` (sección "Compactación y poda" y sección "Handoff y continuidad"), la sección operativa del plugin y del scout se documenta en `docs/architecture.md` ("Capa de exploración y compactación") y el contrato del plugin se describe en su `docstring` como entrada estática. `AGENTS.md` añade `scout`, `context-handoff` y el plugin al mapa de documentos por tarea y al inventario de recursos, describiendo el plugin como entrada estática añadida a `output.context`.
  - El plugin se evalúa cargándolo en una sesión real de OpenCode 1.18.4 (`opencode debug config` lista el plugin con `scope: "local"`; `opencode run --print-logs --log-level INFO --format json` registra la sesión sin errores de importación ni de carga) y por los greps de validación mencionados arriba. No se usa `tsc`, `npx` ni toolchain temporal.
- **Reversibilidad**: fácil. Revertir requiere eliminar `.opencode/agents/scout.md`, `.opencode/commands/review.md`, `.opencode/skills/context-handoff/SKILL.md` y `.opencode/plugins/structured-compaction.ts`, y restaurar el formato de siete secciones en `.opencode/commands/handoff.md`, `docs/ai/context-policy.md`, `AGENTS.md` y `docs/current-state.md`. ADR-001 mantendría su vigencia. Al ser el plugin estrictamente estático, su eliminación no deja estado residual ni dependencias huérfanas en el repositorio.

## ADR-013: validación estática de OpenCode y línea base de evaluación (v0.3.2)

- **Fecha**: 2026-07-23.
- **Estado**: aceptada.
- **Estado inicial confirmado** (cierre de v0.3.1, del que parte esta unidad):
  - Commit vigente: `d87abbb feat: add structured compaction and scout workflow`.
  - CI remota aprobada (matriz 3.12 y 3.14; job `package` con build, verificación y artifact único).
  - Tag `v0.3.1` publicado.
  - Instalación desde tag aprobada (`uv tool install .` y shim local verificados).
  - Working tree limpio al cierre de v0.3.1.
  - ADR-012 vigente y operativo; plugin `structured-compaction` validado por carga y por greps.
- **Contexto**: tras ADR-007 (cuatro quality gates obligatorios) y ADR-012 (capa de exploración y compactación) el repositorio tiene un plugin estático, un agente scout, comandos `/review` y `/handoff`, una skill `context-handoff` y un formato canónico de `docs/current-state.md` con 10 secciones obligatorias. Faltaba (a) un gate automático que detecte regresiones en la forma estructural del plugin sin necesidad de abrir OpenCode; (b) un mecanismo reproducible para registrar evaluaciones manuales, sin telemetría externa y sin consola script; (c) un contrato de extensión para el scout; (d) una prueba manual controlada de la compactación. Tampoco se había decidido qué hacer si el plugin crece más allá de la forma estática conocida.
- **Decisión**:
  - **Quinto gate estático `tools/ai/verify_opencode.py`**. Inspección textual deliberadamente estrecha, sólo biblioteca estándar, sin compilar TypeScript, sin ejecutar OpenCode, sin red, sin modificar archivos. Funciones con `repo_root: Path` explícito; tests herméticos con `tmp_path`. Estructura única `Issue(area, message)`. Tres representaciones del código fuente:
    - **Texto crudo** para validar imports (los strings se conservan para leer el módulo de origen).
    - **Texto sin comentarios** para verificar la clave estructural del hook `experimental.session.compacting` exactamente una vez.
    - **Código con comentarios y contenido de strings enmascarados a espacios** (mediante un scanner léxico pequeño y acotado) para contar `output.context.push`, detectar asignación a `output.prompt` y detectar tokens prohibidos (filesystem, shell, red, logging, persistencia). El scanner reconoce comentarios `//` y `/* */`, strings `'` y `"`, template literals `` ` ``, respeta escapes y conserva saltos de línea; **no** procesa recursivamente `${ ... }` dentro de template literals (limitación declarada).
    Política de red: detectar `fetch(` como token ejecutable; **no** buscar `http`/`https` como subcadenas; bloquear módulos de red por **política de imports** sobre texto crudo.
    Política de filesystem/shell/logging/persistencia: tokens ejecutables concretos más imports prohibidos.
    Verifica además: existencia de `.opencode/agents/scout.md`, `.opencode/commands/review.md`, `.opencode/commands/handoff.md` y `.opencode/skills/context-handoff/SKILL.md`; `docs/current-state.md` contiene exactamente los 10 encabezados canónicos, una vez cada uno y en orden estricto. Los detalles adicionales deben utilizar `###` (tercer nivel). Convención declarada en esta ADR.
    El validador protege la forma estructural del plugin estático conocido. Si el plugin crece (más hooks, más imports, lógica dinámica), este validador puede requerir actualización; eso se declara explícitamente en el docstring del módulo y en esta ADR. **No** sustituye a `/review`.
    Se integra como quinto gate al final de la tupla `GATES` en `tools/ai/verify.py`. El contrato local/CI es único: el job `quality` de `.github/workflows/ci.yml` ejecuta `uv run python tools/ai/verify.py` y el orquestador invoca los cinco gates en orden.
  - **Política de encabezados `docs/current-state.md`** (nueva convención). Los 10 encabezados canónicos deben aparecer exactamente una vez y en orden estricto. Cualquier detalle adicional debe ir bajo `###` (tercer nivel), nunca como `## ` adicional. Esta convención la verifica el quinto gate. Queda registrada en esta ADR.
  - **Registrador manual `tools/ai/record_evaluation.py`**. CLI opt-in, sólo biblioteca estándar. Argumentos obligatorios: `--task`, `--agent`, `--model`, `--provider`, `--result`. Opcionales: `--duration-minutes`, `--escalated` (default `no`), `--corrections`, `--notes`, `--output`, `--write`. `result` ∈ {`approved`, `approved-with-minor-changes`, `changes-required`, `failed`, `inconclusive`}. `duration-minutes`, si está, debe ser finito y ≥ 0 (se rechaza NaN e infinito); si falta, se renderiza literalmente **"no registrada"**. Sin `--write` imprime preview a stdout y no toca el disco. Con `--write`, escritura atómica (archivo temporal en el mismo directorio seguido de `Path.replace()`); preserva el contenido previo; no crea el directorio padre; si el archivo no existe y su directorio sí, lo crea con el encabezado canónico de `evaluations.md`. No genera IDs de sesión, no mide tokens, no añade timestamps precisos: sólo fecha local `YYYY-MM-DD`. Medidas reales anti secreto: no existe `--prompt` (se omite a propósito); límites de longitud (task ≤ 200, agent/model/provider ≤ 100, corrections/notes ≤ 2000); rechazo de caracteres de control (excepto `\n` y `\t`); rechazo de bloques PEM evidentes (`-----BEGIN ... -----END ...`); advertencia explícita en la ayuda de no introducir secretos. **No** se afirma que los límites "detectan secretos"; lo que se afirma es que reducen la superficie de error. **No** se añade consola script: la invocación es `uv run python tools/ai/record_evaluation.py ...`.
  - **Comando `/compact-test`** en `.opencode/commands/compact-test.md`. Prueba manual reproducible y desechable del plugin de compactación. Pasos: crear sesión nueva, no usar información sensible, establecer un fixture mínimo con objetivo, decisión, archivo modificado ficticio, validación, error, enfoque rechazado, divergencia, siguiente acción; disparar la compactación disponible en OpenCode 1.18.4 (umbral automático, o comando soportado si existe); solicitar el checkpoint recuperado; verificar las 10 secciones canónicas; ejecutar `git status --short` (debe estar vacío); clasificar como `approved`, `failed` o `inconclusive`; registrar con `record_evaluation.py`. **No** modifica ADR ni `docs/current-state.md` automáticamente. Si la compactación no se dispara por umbral ni por comando soportado, el resultado se clasifica como `inconclusive` y se registra igualmente. La clasificación `inconclusive` no es un fallo de la prueba: es el resultado esperado cuando OpenCode 1.18.4 no expone un mecanismo soportado para forzar compactación.
  - **Presupuesto contractual del scout** (modificación del contrato de `.opencode/agents/scout.md`). Respuesta normal: aproximadamente ≤ 700 palabras. Exploración amplia solicitada explícitamente: aproximadamente ≤ 1200. Máximo 5 riesgos, 5 hipótesis, 5 preguntas abiertas. No repetir ADR completas; citarlas por identificador. No listar árboles completos salvo petición. Excepción: si el scout detecta un bloqueante, puede superar el límite **solo para no omitirlo** y debe declararlo explícitamente al inicio. La restricción es contractual, no técnica.
  - **Ausencia explícita de telemetría externa y routing automático**. Esta unidad no introduce Cavemem, OpenRouter, Caveman, Ponytail Ultra, routing adaptativo, envío de métricas a servicios externos, bases de datos, SQLite, extracción automática de tokens, nuevos proveedores, permisos por agente nuevos, `tsconfig.json`, ESLint, `tsc`, `npx`, `pre-commit` ni dependencias de runtime. No se modifican `src/python_ai_template/` (más allá del bump de versión al cierre), `tools/new_project.py`, la plantilla (`src/python_ai_template/template/`), `tests/` previos ni la API pública.
  - **Bump de versión 0.3.1 → 0.3.2** al cierre, sólo tras implementación, revisión, documentación y validación local completa con los cinco gates en verde y `verify_wheel.py` aprobado.
- **Alternativas consideradas**:
  - **Compilar TypeScript con `tsc` o `npx` para validar el plugin**: descartado por añadir toolchain temporal y contradecir el principio de cambio mínimo. La inspección textual deliberadamente estrecha es suficiente para la forma estructural del plugin estático conocido.
  - **Validador basado en regex globales que eliminan comentarios y strings indiscriminadamente**: descartado por imprecisión (falsos positivos por menciones en comentarios, falsos negativos por menciones en strings). Se sustituye por el scanner léxico pequeño y acotado y por las tres representaciones explícitas.
  - **Persistencia del resumen de compactación en disco desde el plugin**: ya rechazado por ADR-012; esta ADR no lo reabre.
  - **Consola script para `record_evaluation.py`**: descartado. Se mantiene como invocación explícita `uv run python tools/ai/record_evaluation.py ...` para no añadir dependencias de runtime ni inflar el `pyproject.toml`.
  - **Métricas automáticas de tokens**: descartadas. El registrador no mide tokens; sólo registra fecha local.
  - **Permisos por agente no confirmados en OpenCode 1.18.4**: descartados. El scout sigue operando bajo restricción contractual, no técnica.
  - **Prueba automatizada de la compactación**: descartada por la ausencia de un mecanismo soportado para forzar compactación en OpenCode 1.18.4. La prueba permanece como manual y opt-in.
  - **Detección de secretos "inteligente"**: descartada. Lo que se hace son medidas reales y declaradas (no `--prompt`, límites, control de caracteres, PEM, advertencia). No se promete detección automática de secretos.
- **Consecuencias**:
  - Los quality gates pasan de cuatro a cinco. `tools/ai/verify.py` añade el quinto al final de `GATES`. `AGENTS.md` actualiza la enumeración y la definición de terminado. `.opencode/commands/verify.md` enumera cinco gates y describe `verify_opencode.py`. El job `quality` de CI no se modifica: ya invoca el orquestador y, por transitividad, el nuevo gate.
  - El plugin `structured-compaction` queda bajo inspección textual continua. Si una sesión futura introduce una segunda llamada a `output.context.push`, una asignación a `output.prompt`, un import de `node:fs` o un `fetch(`, el quinto gate lo bloquea antes del merge.
  - `docs/current-state.md` queda bajo inspección de encabezados. Una sesión que rompa el orden canónico o duplique un encabezado queda bloqueada por el quinto gate. Los detalles deben usar `###`.
  - El scout queda acotado contractualmente. La compactación queda con una prueba manual reproducible y con un clasificador explícito (`approved`/`failed`/`inconclusive`). El registro manual queda disponible sin telemetría externa.
  - El plugin **sigue siendo experimental** (ADR-012) y el validador protege su forma actual; **no** garantiza que el plugin siga siendo correcto si su lógica cambia. La protección es estructural, no semántica.
  - La política de compactación y contexto se actualiza en `docs/ai/context-policy.md` ("Compactación y poda" y "Handoff y continuidad"); la sección operativa se documenta en `docs/architecture.md` ("Validación estática de OpenCode y baseline de evaluación"); el inventario se amplía en `AGENTS.md` (mapa de documentos por tarea). `docs/todos.md` registra la unidad completada.
  - No se añaden dependencias de runtime al paquete Python. No se introduce `pre-commit`. No se publica a PyPI ni se crean GitHub Releases. No se modifica el comportamiento del generador.
- **Reversibilidad**: **ADR-013 nunca se elimina como parte de una reversión**. Si la decisión se revierte en el futuro, el código puede revertirse (eliminar `tools/ai/verify_opencode.py` y la entrada en `GATES`; eliminar `tools/ai/record_evaluation.py`, sus tests y `.opencode/commands/compact-test.md`; restaurar `.opencode/agents/scout.md` a su forma previa; ajustar `AGENTS.md`, `verify.md`, `docs/architecture.md` y `docs/ai/context-policy.md`), pero **ADR-013 permanece como registro histórico** y una ADR posterior debe marcarla como "Reemplazada por ADR-NNN" o "Revertida por ADR-NNN". El bump de versión se revierte trivialmente con `git revert`; el paquete no tiene cambios funcionales.
- **Política de fuente canónica y sincronización** (sub-apartado registrado durante la revisión previa a `/review`, 2026-07-23):
  - `src/python_ai_template/template/.opencode/` es la **fuente canónica distribuida** de los artefactos OpenCode. Es lo que se incluye en el wheel y se copia a los proyectos generados con `new-python-project`.
  - `.opencode/` es la **copia operativa** que OpenCode carga sobre este checkout durante el desarrollo.
  - Los siete pares declarados en `SYNC_PAIRS` (`tools/ai/verify_opencode.py`) deben ser byte a byte idénticos: `agents/scout.md`, `commands/review.md`, `commands/handoff.md`, `commands/verify.md`, `commands/compact-test.md`, `skills/context-handoff/SKILL.md`, `plugins/structured-compaction.ts`.
  - `verify_opencode.py` (función `verify_sync`, gate 5) detecta archivos ausentes en uno u otro lado y divergencias byte a byte, emitiendo `[error] sync: <ruta raíz> difiere de <ruta template>` y `[error] sync: falta en raiz|template: <ruta>`.
